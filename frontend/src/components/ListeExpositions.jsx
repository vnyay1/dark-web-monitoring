/**
 * Liste d'expositions filtrable, partagee par les pages Expositions et
 * Archives : meme tableau, meme tri, memes filtres, seule la source de
 * donnees change. Les deux ecrans etaient identiques a trois lignes pres ;
 * les dupliquer aurait garanti qu'ils divergent a la premiere evolution.
 *
...
import { EnTeteTri, useTri } from "./tri";
...
/**
 * charger       : (filtres) => Promise, l'appel d'API de la page.
 * appartient    : (exposition) => bool, vrai tant que l'exposition a sa
 *                 place dans CETTE liste. Un changement de statut qui la
 *                 fait basculer la retire de l'affichage, avec un message
 *                 disant ou elle est partie (messageSortie).
 * titreVide/aideVide : etat vide propre a la page.
 */
export default function ListeExpositions({
  titre,
  sousTitre,
  etiquette,
  charger,
  ...
  async function changerStatut(exposition, statut) {
    try {
      ...
      const sortie = !appartient(reponse.exposition);

      // Mise a jour locale plutot que rechargement : la position dans la
      // liste est conservee. Une exposition qui n'a plus sa place ici est
      // retiree, et le compteur suit - sans quoi la ligne resterait
      // affichee jusqu'au prochain chargement, en contradiction avec le
      // filtre du serveur.
      setDonnees((precedent) => ({
        ...precedent,
        total: sortie ? Math.max(0, precedent.total - 1) : precedent.total,
        expositions: sortie
          ? precedent.expositions.filter((e) => e.id !== exposition.id)
          : precedent.expositions.map((e) => (e.id === exposition.id ? reponse.exposition : e)),
      }));

      ajouter(
        sortie
          ? messageSortie(exposition.nom_entite, statut)
          : `${exposition.nom_entite} : statut « ${LIBELLE_STATUT[statut] || statut} » enregistré.`,
      );
      return true;
    } catch (e) {
  ...
  return (
    <>
      ...
      {aide && (
        <p className="texte-aide espace-bas" id="aide-liste">
          {aide}
        </p>
      )}
      ...
      {chargement ? (
        ...
        <Vide titre={titreVide}>
          {puces.length > 0 ? "Retirez un filtre ou élargissez la période." : aideVide}
        </Vide>
      ) : (
        <>
          ...
          <div className="table-wrap tableau-cartes">
            <table className="data">
              <caption className="sr-only">
                {etiquette}
                {puces.length > 0 ? `, filtrées (${puces.map((p) => p.libelle).join(", ")})` : ""}
              </caption>
              <thead>
                <tr>
                    ...
                    <td data-label="Statut">
                      {peutChangerStatut ? (
                        <ChoixStatut
                          compact
                          statut={e.statut}
                          // Liste COMPLETE, distincte de celle du filtre :
                          // c'est ici qu'on classe en faux positif ou qu'on
                          // cloture, donc qu'on archive.
                          statuts={referentiels?.statuts_modifiables || []}
